class path: #Class names start with capitals. When in doubt, read the PEP 8 style guide. https://www.python.org/dev/peps/pep-0008/
	...
	def __init__(self, input):
		...
		self.direction = 0
		self.step_number = 0
		self.ending_locations = [[0,0]]		self.crosses = []

	def dir_letter(self):
		d = "NESW" #define class constants right underneath "class Path:". PEP 8 says use all caps for constants
		# class Path:
		#	DIRECTIONS = 'NESW'
	...
	def step(self,cross):
		...
		m = int(s[1::]) #this is commonly just written as s[1:] <-- only one colon
		...
		if cross == True:
			...
			if c != []: # just write "if c:"
				...
				for i in c:
					...
					self.move(move)
					moved += move
					self.crosses.append(self.current_position[::]) #Don't use colonss unlesss you're slicing. self.current_position[::] == self.current_position
	...
	def cross_check(self):
		...
		if ahead != []:		
			for pos in left:
				# The next lines until the outdent should all be in a function that you invoke with left or right. The giveaway was that you just copied and pasted the code for left and right.
				try:
