...
class ReactionSystem:
    ...
    def max_fuel(self, ore=1000000000000):
        per_fuel = self.ore_needed('FUEL')
        num_reactions = 1
        ore_consumed = self.ore_needed('FUEL', num_reactions)
        while ore > ore_consumed:
            new_reactions = math.floor((ore - ore_consumed) / per_fuel)
            if new_reactions > 0:
                num_reactions += new_reactions
            else:
                num_reactions += 1
            ore_consumed = self.ore_needed('FUEL', num_reactions)
...
def part2(input):
    """
    >>> import aoc_input as inp; part2(inp.read(14))
    5650230
    """
    rs = ReactionSystem(input)
    ratio = rs.ore_needed('FUEL')
    return rs.max_fuel()
